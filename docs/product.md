...
- A user-directed **naming studio** that opens from the full 510-word etymology table (expanded 2026-08-26 from 270 across four themed batches — nature/virtue/harvest/lux — each entry carrying attested era forms, semantic drift, tone, syllables, and curated brand marks). Editors pick up to six base words to see every language-era form with meaning, splice openings/endings with a live preview, review ranked portmanteau/compound/suffix suggestions, and add base words, curated marks, or splices to an additive board. Per-brand session persistence (picks + mixing eras survive navigation), a deterministic "Surprise me" starter pair, and board-aware dedupe keep the flow effortless. Availability checks return `.com`, `.io`, and `.co` RDAP statuses alongside shortlist and select controls.
...
- A public, sign-in-free **etymology lab** at `/names`: a focused name generator over the same 510-word corpus. One matrix table — root English words as rows, language eras as columns, curated marks as an end column. An empty search shows all 510 words; typing narrows the rows. Clicking any era cell or curated mark selects it, and the **Selected** view lists only the chosen etymology words with their sibling variations and an end column for idea notes. Selections and notes persist in browser `localStorage`. The page ships the full corpus view-model to the client so search and selection are instant; it stores nothing server-side.
