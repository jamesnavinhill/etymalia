...
2. **Naming studio**
   ...
   - [x] Add the standalone etymology lab at `/names` (public, no auth, no server writes): one matrix table with root words as rows and language eras as columns — an empty search shows all 510 words, typing narrows the rows — with per-cell selection of era forms and curated marks, and a Selected view pairing each chosen word with its variations and an idea-notes end column. State persists in browser `localStorage`; the page ships a cached corpus view-model so search and selection are instant.
...
| 2026-09-11 | Added the public etymology lab at `/names`: a focused, sign-in-free name generator over the 510-word corpus. One matrix table with root words as rows and language eras as columns (all words shown with an empty search), per-cell selection of forms and curated marks, and a Selected view with sibling variations and idea notes. State persists in browser `localStorage`. Local web type check and production build pass. |
